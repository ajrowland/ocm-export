const subTransform = (
  ...
  { strict, source, actions }: { source: any; strict: any; actions: any }
) => {
  ...
  properties.forEach((property) => {
    ...
    if (transformerProperty.constructor === String) {
      let res = object ? object[transformerProperty.toString()] : undefined;
      if (!res) {
        res = object;
      ...
      subObject[property] = subTransform(object, transformerProperty, {
        source: subObject[property],
        strict,
        actions,
      });
...
export default (
  object: { [key: string]: any },
  transformer: { [key: string]: any },
  options = { strict: false, source: undefined, actions: {} }
) => {
  const { strict, source, actions } = options;
